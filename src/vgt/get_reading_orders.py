...
def add_no_token_segments(segments, no_token_segments):
    if segments:
        # Vectorized distance calculation for all no_token_segments at once
...
def filter_and_sort_segments(page, tokens_by_segments, types, token_to_order_map=None):
    ...
    order = {seg: get_average_reading_order_for_segment(page, tokens_by_segments[seg], token_to_order_map) for seg in filtered_segments}
...
def get_ordered_segments_for_page(segments_for_page: list[PdfSegment], page: PdfPage):
    ...
    spatial_index = None
    if RTREE_AVAILABLE and len(segments_for_page) > 10:  # Only use for larger segment counts
